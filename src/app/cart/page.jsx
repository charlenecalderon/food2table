...
export default function CartPage() {
  ...
  const handleAdd = async (id) => {
    ...
    if (res.ok) setCartItems(cartItems.map((i) => i.id === id ? { ...i, quantity: i.quantity + 1 } : i));
  };
  ...
  const handleMinus = async (id) => {
    ...
    if (res.ok) setCartItems(cartItems.map((i) => i.id === id ? { ...i, quantity: Math.max(1, i.quantity - 1) } : i));
  };
  ...
  const handleRemove = async (id) => {
    ...
    if (res.ok) setCartItems(cartItems.filter((i) => i.id !== id));
  };
