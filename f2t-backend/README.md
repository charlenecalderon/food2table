...
1. Start database
   ...
   - `f2t-backend` (use `cd f2t-backend` [Windows] or `pwd` [Mac])
...
5. Files that might need to be imported to authenticate information:
   - `/plugins/authentication.ts`
   - `/routes/authentication.ts`

6. Environment variables used in `/plugins/authentication.ts`:
...
**NOTE:** The `prisma/schema.prisma` file has information that we want to create/set/update for our users. Let me know on Discord if there is any other information we should consider.
...
- The JWT is stored in the cookie named `session`
